...
<body>
    ...
    <script nonce="${nonce}">
        (function() {
            ...
            molecularData.atoms.forEach(function(atom) { centerX += atom.x; centerY += atom.y; centerZ += atom.z; });
            centerX /= molecularData.atoms.length; centerY /= molecularData.atoms.length; centerZ /= molecularData.atoms.length;
            const COVALENT_RADII = { H: 0.31, He: 0.28, Li: 1.28, Be: 0.96, B: 0.84, C: 0.76, N: 0.71, O: 0.66, F: 0.57, Ne: 0.58, Na: 1.66, Mg: 1.41, Al: 1.21, Si: 1.11, P: 1.07, S: 1.05, Cl: 1.02, Ar: 1.06, K: 2.03, Ca: 1.76, Sc: 1.70, Ti: 1.60, V: 1.53, Cr: 1.39, Mn: 1.39, Fe: 1.32, Co: 1.26, Ni: 1.24, Cu: 1.32, Zn: 1.22, Ga: 1.22, Ge: 1.20, As: 1.19, Se: 1.20, Br: 1.20, Kr: 1.16, Rb: 2.20, Sr: 1.95, I: 1.39, Cs: 2.44, Ba: 2.15 };
            function getAtomRadius(element) { const r = COVALENT_RADII[element] || 1.50; return r * 0.5; }
            const atomMeshes = [];
            function createAtom(atom) {
                const radius = getAtomRadius(atom.element);
                const geometry = new THREE.SphereGeometry(radius, 32, 24);
                const material = new THREE.MeshPhongMaterial({ color: new THREE.Color(atom.color), shininess: 80, specular: 0x444444 });
            ...
            molecularData.atoms.forEach(function(atom, i) { atom.index = i; createAtom(atom); });
            molecularData.bonds.forEach(function(bond) { createBond(bond); });
            let maxDist = 0;
            molecularData.atoms.forEach(function(atom) { const dx = atom.x - centerX; const dy = atom.y - centerY; const dz = atom.z - centerZ; const dist = Math.sqrt(dx*dx + dy*dy + dz*dz); if (dist > maxDist) maxDist = dist; });
            ...
            let rotationX = 0, rotationY = 0, panX = 0, panY = 0;
            let cameraDistance = initialCameraDistance;
            function resetView() { rotationX = 0; rotationY = 0; panX = 0; panY = 0; cameraDistance = initialCameraDistance; camera.position.set(0, 0, cameraDistance); updateMoleculeTransform(); }
            resetBtn.addEventListener('click', resetView);
            const canvas = renderer.domElement;
            canvas.addEventListener('mousedown', function(e) { if (e.button === 0) { isRotating = true; } else if (e.button === 1 || e.button === 2) { isPanning = true; } previousMousePosition = { x: e.clientX, y: e.clientY }; e.preventDefault(); });
            canvas.addEventListener('mousemove', function(e) {
                const deltaMove = { x: e.clientX - previousMousePosition.x, y: e.clientY - previousMousePosition.y };
                if (isRotating) { rotationY += deltaMove.x * 0.005; rotationX += deltaMove.y * 0.005; updateMoleculeTransform(); }
                if (isPanning) { panX += deltaMove.x * 0.01 * (cameraDistance / 20); panY -= deltaMove.y * 0.01 * (cameraDistance / 20); updateMoleculeTransform(); }
            ...
            canvas.addEventListener('wheel', function(e) { e.preventDefault(); const zoomFactor = e.deltaY > 0 ? 1.1 : 0.9; cameraDistance *= zoomFactor; cameraDistance = Math.max(1, Math.min(500, cameraDistance)); camera.position.z = cameraDistance; }, { passive: false });
            ...
            function checkAtomHover(e) {
                ...
                if (intersects.length > 0) { const obj = intersects[0].object; const elem = obj.userData.element; const idx = obj.userData.index; const atom = molecularData.atoms[idx]; tooltipEl.textContent = elem + (idx + 1) + '  (' + atom.x.toFixed(4) + ', ' + atom.y.toFixed(4) + ', ' + atom.z.toFixed(4) + ')'; tooltipEl.style.display = 'block'; tooltipEl.style.left = (e.clientX - container.getBoundingClientRect().left + 15) + 'px'; tooltipEl.style.top = (e.clientY - container.getBoundingClientRect().top - 10) + 'px'; }
                else { tooltipEl.style.display = 'none'; }
            }
            let touchStartDistance = 0;
            canvas.addEventListener('touchstart', function(e) { e.preventDefault(); if (e.touches.length === 1) { isRotating = true; previousMousePosition = { x: e.touches[0].clientX, y: e.touches[0].clientY }; } else if (e.touches.length === 2) { isRotating = false; const dx = e.touches[0].clientX - e.touches[1].clientX; const dy = e.touches[0].clientY - e.touches[1].clientY; touchStartDistance = Math.sqrt(dx * dx + dy * dy); } }, { passive: false });
            canvas.addEventListener('touchmove', function(e) { e.preventDefault(); if (e.touches.length === 1 && isRotating) { const deltaMove = { x: e.touches[0].clientX - previousMousePosition.x, y: e.touches[0].clientY - previousMousePosition.y }; rotationY += deltaMove.x * 0.005; rotationX += deltaMove.y * 0.005; updateMoleculeTransform(); previousMousePosition = { x: e.touches[0].clientX, y: e.touches[0].clientY }; } else if (e.touches.length === 2) { const dx = e.touches[0].clientX - e.touches[1].clientX; const dy = e.touches[0].clientY - e.touches[1].clientY; const dist = Math.sqrt(dx * dx + dy * dy); if (touchStartDistance > 0) { const zoomFactor = touchStartDistance / dist; cameraDistance *= zoomFactor; cameraDistance = Math.max(1, Math.min(500, cameraDistance)); camera.position.z = cameraDistance; } touchStartDistance = dist; } }, { passive: false });
            canvas.addEventListener('touchend', function(e) { isRotating = false; if (e.touches.length < 2) { touchStartDistance = 0; } });
