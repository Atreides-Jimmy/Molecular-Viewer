"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.detectBonds = detectBonds;
exports.ensureBonds = ensureBonds;
const COVALENT_RADII = {
    H: 0.31, He: 0.28,
    Li: 1.28, Be: 0.96, B: 0.84, C: 0.76, N: 0.71, O: 0.66, F: 0.57, Ne: 0.58,
    Na: 1.66, Mg: 1.41, Al: 1.21, Si: 1.11, P: 1.07, S: 1.05, Cl: 1.02, Ar: 1.06,
    K: 2.03, Ca: 1.76, Sc: 1.70, Ti: 1.60, V: 1.53, Cr: 1.39, Mn: 1.39, Fe: 1.32,
    Co: 1.26, Ni: 1.24, Cu: 1.32, Zn: 1.22, Ga: 1.22, Ge: 1.20, As: 1.19, Se: 1.20,
    Br: 1.20, Kr: 1.16,
    Rb: 2.20, Sr: 1.95, Y: 1.90, Zr: 1.75, Nb: 1.64, Mo: 1.54, Tc: 1.47, Ru: 1.46,
    Rh: 1.42, Pd: 1.39, Ag: 1.45, Cd: 1.44, In: 1.42, Sn: 1.39, Sb: 1.39, Te: 1.38,
    I: 1.39, Xe: 1.40,
    Cs: 2.44, Ba: 2.15, La: 2.07, Ce: 2.04, Pr: 2.03, Nd: 2.01, Pm: 1.99, Sm: 1.98,
    Eu: 1.98, Gd: 1.96, Tb: 1.94, Dy: 1.92, Ho: 1.92, Er: 1.89, Tm: 1.90, Yb: 1.87,
    Lu: 1.87, Hf: 1.75, Ta: 1.70, W: 1.62, Re: 1.51, Os: 1.44, Ir: 1.41, Pt: 1.36,
    Au: 1.36, Hg: 1.32, Tl: 1.45, Pb: 1.46, Bi: 1.48, Po: 1.40, At: 1.50, Rn: 1.50,
    Fr: 2.60, Ra: 2.21, Ac: 2.15, Th: 2.06, Pa: 2.00, U: 1.96, Np: 1.90, Pu: 1.87,
    Am: 1.80, Cm: 1.69, Bk: 1.68, Cf: 1.68, Es: 1.65, Fm: 1.67, Md: 1.73, No: 1.76,
    Lr: 1.61, Rf: 1.57, Db: 1.49, Sg: 1.43, Bh: 1.41, Hs: 1.34, Mt: 1.29, Ds: 1.28,
    Rg: 1.21, Cn: 1.22, Nh: 1.36, Fl: 1.43, Mc: 1.62, Lv: 1.75, Ts: 1.65, Og: 1.57
};
const DEFAULT_RADIUS = 1.50;
const BOND_TOLERANCE = 0.45;
function getCovalentRadius(element) {
    const e = element.charAt(0).toUpperCase() + element.slice(1).toLowerCase();
    return COVALENT_RADII[e] || DEFAULT_RADIUS;
}
function distance(a1, a2) {
    const dx = a1.x - a2.x;
    const dy = a1.y - a2.y;
    const dz = a1.z - a2.z;
    return Math.sqrt(dx * dx + dy * dy + dz * dz);
}
function estimateBondOrder(d, r1, r2) {
    const sumR = r1 + r2;
    if (sumR < 0.01)
        return 1;
    const ratio = d / sumR;
    if (ratio <= 0.78)
        return 3;
    if (ratio <= 0.88)
        return 2;
    return 1;
}
function detectBonds(atoms) {
    const bonds = [];
    const n = atoms.length;
    for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
            const r1 = getCovalentRadius(atoms[i].element);
            const r2 = getCovalentRadius(atoms[j].element);
            const maxDist = r1 + r2 + BOND_TOLERANCE;
            const d = distance(atoms[i], atoms[j]);
            if (d > 0 && d <= maxDist) {
                const order = estimateBondOrder(d, r1, r2);
                bonds.push({ atom1: i, atom2: j, order });
            }
        }
    }
    return bonds;
}
function ensureBonds(data) {
    if (data.hasExplicitBonds && data.bonds.length > 0) {
        return data;
    }
    const bonds = detectBonds(data.atoms);
    return {
        ...data,
        bonds,
        hasExplicitBonds: false
    };
}
//# sourceMappingURL=bondDetector.js.map